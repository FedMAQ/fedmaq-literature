...
**Policy:** add an entry when a milestone lands (kg/ bundle restructure, phase
completion, de-overclaim pass) — not per-commit.
...
## Earlier
Restructured around the OKF (Open Knowledge Format) bundle, removed the local
vector-RAG stack, and migrated agent tooling from Cursor to Claude Code.
