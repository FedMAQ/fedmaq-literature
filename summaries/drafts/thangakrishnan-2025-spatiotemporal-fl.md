# Research Summary: Spatiotemporal Federated Learning for Privacy-Preserving Load Forecasting and Appliance Scheduling
...
$$
\min_{s_i, m_i} J_i = \omega_1 \sum_{t=1}^{T} \lambda_t P_{\text{grid},it} + \omega_2 \sum_{m \in A_i} P_{\text{penalty}}^{A_{i,m}} - \omega_3 \max_{t \in T} L_{it}
$$

where:
- $\omega_1, \omega_2, \omega_3$ are weights summing to 1
- $\lambda_t$ is real-time electricity price
- $P_{\text{penalty}}^{A_{i,m}}$ quantifies user discomfort from scheduling delays
- $L_{it}$ is total load (minimizing peak improves grid stability)
