## Research Summary: Communication-Efficient Federated Learning via Knowledge Distillation (FedKD)
...
Server receives $g_i$ from clients, reconstructs full matrices via SVD factors, then computes:
$$
g = \frac{1}{N} \sum_{i=1}^N g_i
$$
The global gradient $g$ is then factorized again using SVD (with the same $T(t)$) and distributed to clients for local mentee update:
$$
\Theta^s \leftarrow \Theta^s - \eta_s \cdot g
$$
...
- **System Assumptions:**
    ...
    - **Computation Increase:** Clients must maintain and train both a large mentor and a small mentee, roughly doubling local computation costs compared to standard FL. This may be prohibitive for low-resource edge devices.
